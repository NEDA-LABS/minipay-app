/**
 * Test Script: Send Payment Refunded Email
 * 
 * Usage:
 * 1. Update the TEST_EMAIL constant with your email
 * 2. Run: npx ts-node test/email/send-payment-refunded-test.ts
 */
...
const testPaymentData = {
  transactionId: 'test-refund-' + Date.now(),
  walletAddress: TEST_WALLET_ADDRESS,
  amount: '100.00',
  currency: 'NGN',
  accountName: 'John Doe',
  accountNumber: '1234567890',
  refundReason: 'Transaction could not be completed due to insufficient bank account balance.',
};
...
async function sendTestEmail() {
  ...
  try {
    console.log('🚀 Starting Payment Refunded Email Test...\n');
    ...
    // Step 3: Send email
    console.log('\n📧 Step 2: Sending payment refunded email...');
    ...
    console.log('Reason:', testPaymentData.refundReason);

    const paymentNotificationService = createPaymentNotificationService(prisma);
    const result = await paymentNotificationService.sendPaymentRefundedEmail(testPaymentData);
...
if (!TEST_EMAIL || TEST_EMAIL.includes('your-email')) {
  ...
  console.error('   Open test/email/send-payment-refunded-test.ts and set your email address.');
  process.exit(1);
}
...
console.log('╔════════════════════════════════════════════════════════════╗');
console.log('║       Payment Refunded Email Test                         ║');
console.log('╚════════════════════════════════════════════════════════════╝\n');

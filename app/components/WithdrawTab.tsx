...
export default function WithdrawTab({ walletAddress }: WithdrawTabProps) {
  ...
  const handleContinue = () => {
    if (selectedCountry && selectedChain) {
      setStep('form');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <AnimatePresence mode="wait">
            ...
            <Card className="bg-gradient-to-br from-slate-900/95 via-slate-900/90 to-slate-800/95 backdrop-blur-xl border border-slate-700/60 shadow-2xl !rounded-3xl overflow-hidden">
              <CardHeader className="pb-6 pt-7 px-7 bg-gradient-to-br from-purple-500/5 via-transparent to-transparent">
                <CardTitle className="text-xl text-white flex items-center gap-3 font-semibold">
                  ...
                  Select Country & Chain
                </CardTitle>
                {/* <CardDescription className="text-slate-400 text-sm mt-2">
                  Choose your withdrawal destination and blockchain network
                </CardDescription> */}
              </CardHeader>
              <CardContent className="space-y-6 px-7 pb-7">
                ...
                {/* Chain Selection - Hidden for Indonesia */}
                ...
                <Button
                  ...
                  disabled={!selectedCountry || (selectedCountry.id !== 'indonesia' && !selectedChain)}
                  className="w-full bg-gradient-to-r from-purple-600 via-purple-500 to-violet-600 hover:from-purple-700 hover:via-purple-600 hover:to-violet-700 text-white font-semibold py-6 rounded-xl shadow-xl shadow-purple-500/25 hover:shadow-2xl hover:shadow-purple-500/40 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none transition-all duration-300 mt-4 transform hover:scale-[1.02] active:scale-[0.98]"
                >
        ...
        {step === 'form' && selectedCountry && (selectedCountry.id === 'indonesia' || selectedChain) && (
          <motion.div
            key="form"
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <div className="space-y-4">
              {/* Selected Country & Chain Badge */}
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-2 px-4 py-2 bg-slate-800/80 backdrop-blur-sm border border-slate-600/50 rounded-full shadow-sm">
                    <span className="text-lg">{selectedCountry.flag}</span>
                    <span className="text-sm font-medium text-slate-200">{selectedCountry.name}</span>
                  </div>
                  {selectedChain && (
                    <>
                      <ChevronRight className="w-4 h-4 text-slate-400" />
                      <div className="flex items-center gap-2 px-4 py-2 bg-slate-800/80 backdrop-blur-sm border border-slate-600/50 rounded-full shadow-sm">
                        <span className="text-sm font-medium text-slate-200">{selectedChain.name}</span>
                      </div>
                    </>
                  )}
                </div>
                <Button
                  ...
                  size="sm"
                  onClick={handleBack}
                  className="text-slate-400 hover:text-purple-400 hover:bg-slate-800/50 rounded-full transition-colors"
                >
                  {selectedCountry.id === 'indonesia' ? 'Change Country' : 'Change Chain'}
                </Button>
              </div>

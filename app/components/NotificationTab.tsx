    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="relative hover:bg-slate-500 transition-colors duration-200 px-2 sm:px-3"
          aria-label="Notifications"
        >
          <Bell className="h-3 w-3 sm:h-4 sm:w-4 text-slate-100" />
          {unreadCount > 0 && (
            <Badge 
              variant="destructive" 
              className="absolute -top-1 -right-1 sm:-top-2 sm:-right-2 h-4 w-4 sm:h-5 sm:w-5 flex items-center justify-center p-0 text-[8px] sm:text-xs font-bold bg-red-500 hover:bg-red-500"
            >

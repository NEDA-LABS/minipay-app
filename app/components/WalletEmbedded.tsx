  ...
  useEffect(() => {
    ...
    const loadBalances = async () => {
      ...
      try {
        ...
        // Add token balances
        const relevantTokens = stablecoins.filter(
        ...
        for (const token of relevantTokens) {
          try {
            const decimals = typeof token.decimals === 'object' 
              ? (token.decimals as any)[active.id] ?? 6 
              : token.decimals;
            
            const balance = await publicClient?.readContract({
              address: token.addresses[active.id] as `0x${string}`,
              abi: ERC20_ABI,
            ...
            if (formatted > 0) {
              rows.push({
                ...
                address: token.addresses[active.id],
                decimals,
                isNative: false
              });
            }
    ...
    loadBalances();
  }, [address, active, publicClient, isOpen]);
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {isLoading && balances.length === 0 ? (
                      ...
                      {balances.map((balance, index) => (
                        <div key={`${balance.symbol}-${index}`} 
                          ...
                          <div className="text-right">
                            <div className="font-semibold text-gray-900">{balance.balance}</div>
                            <div className="text-sm text-gray-500">≈ ${balance.usd}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500">No supported tokens found on {active.name}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 p-4 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition font-medium"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 p-4 bg-green-600 text-white rounded-xl hover:bg-green-700 transition font-medium"
                  >
            ...
            {activeTab === 'send' && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-gray-900 ${
                      sendTo && !isValidAddress ? 'border-red-300 bg-red-50' : 'border-gray-300'
                    }`}
                  />
                ...
                <button
                  ...
                  className="w-full bg-blue-600 text-white py-4 rounded-xl hover:bg-blue-700 transition font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
                ...
                <div className="border-t pt-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <button
                      ...
                      className="w-full bg-green-600 text-white py-4 rounded-xl hover:bg-green-700 transition font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                      {isLoading ? (
                        <>
                          <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                ...
                <div className="border border-yellow-200 bg-yellow-50 p-6 rounded-xl">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="font-semibold text-yellow-800 mb-2">Export Private Key</h4>
                ...
                <div className="bg-gray-50 p-6 rounded-xl">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="text-gray-900 font-medium">{active.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Chain ID:</span>
                      <span className="text-gray-900 font-medium">{active.id}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Currency:</span>
                      <span className="text-gray-900 font-medium">{active.nativeCurrency.symbol}</span>
                    </div>
                    {active.blockExplorers?.default && (
                      <div className="flex justify-between items-center">
                        <span className="text-gray-600">Explorer:</span>
                        <a
                          href={`${active.blockExplorers.default.url}/address/${address}`}
                          target="_blank"

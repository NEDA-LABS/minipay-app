...
export function RedeemForm() {
  ...
  const onSubmit = (form: RedeemFormData) => {
    ...
    redeemMutation.mutate(form, { onSettled: () => setIsSubmitting(false) });
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div>
          <div className="flex justify-between items-center mb-1">
            ...
            <div className="text-sm text-gray-500">
              <span className="font-medium">{isLoadingBalance ? '...' : parseFloat(balance || '0').toLocaleString('id-ID')} IDRX</span>
              <Button variant="link" type="button" onClick={handleMaxAmount} className="h-auto p-0 ml-2 text-sm font-semibold text-blue-600">Max</Button>
            </div>
          </div>
          <div className="relative">
            <Input id="amount" type="number" placeholder="Amount" className="w-full pl-4 pr-32 py-6 border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500" {...register('amount')} />
        ...
        <div>
          ...
          <div className="relative mt-1">
            <Input id="receiveAmount" type="number" placeholder="Amount" className="w-full pl-4 pr-24 py-6 border-gray-300 rounded-lg bg-gray-50" value={watch('amount')} readOnly />
            <div className="absolute inset-y-0 right-0 flex items-center pr-3">
        ...
        <div className="text-sm text-gray-500">Exchange rate: <span className="font-medium text-gray-700">1 = 1 IDR</span></div>
        ...
        <Button type="submit" className="w-full flex items-center justify-center bg-gray-100 hover:bg-gray-200 text-gray-500 font-semibold py-3 rounded-lg disabled:opacity-50" disabled={isSubmitting || redeemMutation.isPending || isLoadingAccounts}>
